...
namespace SolutionConverter
{
    ...
    public partial class FrmMain : Form
    {
        ...
        private void OpenFolderbtn_Click(object sender, EventArgs e)
        {
            ...
            List<string> files = DirSearch("c:\\Temp", new List<string>());

            foreach (string file in files)
            {
                this.solutionPath = file;
                this.LoadSolution(file);
        ...
        {
            foreach (string f in Directory.GetFiles(sDir, "*.sln"))
            {
                string extension = Path.GetExtension(f);
                if (extension != null && (extension.Equals(".sln")))
                {
                    files.Add(f);
                }
            }
