    ...
    public partial class Form1 : Form
    {
        ...
        private void CreateConstraints()
        {
            ...
            UniqueConstraint pkCustomer =
                new UniqueConstraint(new DataColumn[] { dtCustomer.Columns["CustomerID"] }, true);
            dtCustomer.Constraints.Add(pkCustomer);
            //建立出貨單資料表的主索引鍵
            UniqueConstraint pkDelivery =
                new UniqueConstraint(new DataColumn[] { dtDelivery.Columns["DeliveryID"] }, true);
        ...
        //取得關聯的子資料表
        private void btnGetChildRow_Click(object sender, EventArgs e)
        {
            DataRow drCustomer;
            drCustomer = dtCustomer.Rows[dataGridView1.CurrentCell.RowIndex];
            listBox1.Items.Clear();
            listBox1.Items.Add("客戶編號" + drCustomer["CustomerID"] + "的出貨單:");
            foreach (DataRow drDeliverys in drCustomer.GetChildRows(
                                            "FK_Customer_Delivery"))
            {
                listBox1.Items.Add("出貨單號:" + drDeliverys["DeliveryID"]);
            }
        }
        ...
        private void btnProperty_Click(object sender, EventArgs e)
        {
            ...
            objRelation = (DataRelation)this.dsXIN.Relations["FK_Customer_Delivery"];
            listBox1.Items.Add("關聯子資料表的欄位:");
            foreach (DataColumn col in objRelation.ChildColumns)
            {
                listBox1.Items.Add(objRelation.ChildColumns[0].ColumnName);
            }
            listBox1.Items.Add("");
            listBox1.Items.Add("子資料表的條件約束名稱:" + 
                               objRelation.ChildKeyConstraint.ConstraintName);
            listBox1.Items.Add("");
            listBox1.Items.Add("子資料表名稱:" + objRelation.ChildTable.TableName);
            listBox1.Items.Add("");
            listBox1.Items.Add("關聯父資料表的欄位:");
            foreach (DataColumn col in objRelation.ParentColumns)
            {
                listBox1.Items.Add(objRelation.ChildColumns[0].ColumnName);
            }
            listBox1.Items.Add("");
            listBox1.Items.Add("父資料表的條件約束名稱:" + 
                               objRelation.ParentKeyConstraint.ConstraintName);
            listBox1.Items.Add("");
            listBox1.Items.Add("父資料表名稱:" + objRelation.ParentTable.TableName);
        }
    }
}
