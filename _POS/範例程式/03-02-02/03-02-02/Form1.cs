...
namespace _03_02_02
{
    public partial class Form1 : Form
    {
        ...
        {
            //建立客戶資料表的主索引鍵
            UniqueConstraint pkCustomer = new UniqueConstraint(new DataColumn[] { dtCustomer.Columns["CustomerID"] }, true);
            dtCustomer.Constraints.Add(pkCustomer);
            //建立出貨單單資料表的主索引鍵
            UniqueConstraint pkDelivery= new UniqueConstraint(new DataColumn[] { dtDelivery.Columns["DeliveryID"] }, true);
            dtDelivery.Constraints.Add(pkDelivery);
            //建立客戶資料表與出貨單單資料表的資料完整性
            ForeignKeyConstraint fkCustomer_Delivery = new ForeignKeyConstraint("FK_Customer_Delivery", dtCustomer.Columns["CustomerID"], dtDelivery.Columns["CustomerID"]);
            dtDelivery.Constraints.Add(fkCustomer_Delivery);
        }

        private void SetUpdateDeleteRule()
        {
            //取出dtDelivery的ForeignKeyConstraint
            ForeignKeyConstraint fkConstraint = (ForeignKeyConstraint)this
                                .dtDelivery.Constraints["FK_Customer_Delivery"];
            if (radioCascade.Checked)
            {
                //Cascade
                fkConstraint.UpdateRule = Rule.Cascade;
                fkConstraint.DeleteRule = Rule.Cascade;
            } 
            else if (radioSetNull.Checked) 
            {
                //SetNull
                fkConstraint.UpdateRule = Rule.SetNull;
                fkConstraint.DeleteRule = Rule.SetNull;
            } 
            else if (radioSetDefault.Checked) 
            {
                //SetDefault
        ...
        private void radioCascade_Click(object sender, EventArgs e)
        {
            //Cascade
            SetUpdateDeleteRule();
        }

        private void radioSetNull_Click(object sender, EventArgs e)
        {
            //SetNull
            SetUpdateDeleteRule();        
        }

        private void radioSetDefault_Click(object sender, EventArgs e)
        {
            //SetDefault
            SetUpdateDeleteRule();
        }
