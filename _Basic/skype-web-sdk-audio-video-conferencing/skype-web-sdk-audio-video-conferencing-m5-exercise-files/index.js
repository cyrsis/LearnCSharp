...
$(function () {
    ...
    if (/chrom(e|ium)/.test(navigator.userAgent.toLowerCase())) {
        log("<font color=red><b>Information:</b> Google Chrome detected!</font>");
        log("<font color=red>Google disabled NPAPI on 1st September 2015, and unfortunately the Video and Collaboration features of the Skype Web SDK is not supported on Chrome at this time.</font>");
        return;
    }
    ...
    //clicks the "start own video" button
    ...
    window.add_participant_to_conversation = function (sipaddress) {
        conversation.addParticipant("sip:" + sipaddress).then(function () {
            ...
            conversation.videoService.start().then(function () {
                ...
                log("Video added to preview window");
                conversation.selfParticipant.video.channels(0).stream.source.sink.container(document.getElementById("localStream"));
            });

            conversationParticipant = conversation.participants(participant_id - 1);

            //watch the state of the remote participants video. When it's connected, show it in the div
            conversationParticipant.video.state.changed(function (state) {
                if (state == 'Connected') {
                    log("Participant video status = " + state)
                    conversationParticipant.video.channels(0).stream.source.sink.container(document.getElementById("remoteStream"));
                    log("Starting");
                    conversationParticipant.video.channels(0).isStarted.set(true);
                }
                if (state == 'Disconnected') {
                    log("Participant video status = " + state)
                    conversationParticipant.video.stop().then(function () {
                        log("Video stopped.");
